...
def imshow_det_bboxes(img,
    ...
    polygons = []
    color = []
    for i, (segm, label) in enumerate(zip(segms, labels)):
        label_text = class_names[
            label] if class_names is not None else f'class {label}'
...
def imshow_det_masks(img,
    ...
    # segms = np.zeros((3, 427, 640))
    # segms[0, 60:90, 30:100] = 1
    # segms[1, 200:300, 400:500] = 1
    # segms[2, 400:410, 200:300] = 1
    ...
    width, height = img.shape[1], img.shape[0]
    img = np.ascontiguousarray(img)
    # img = np.ascontiguousarray(np.zeros(img.shape, dtype=np.uint8))
    ...
    def center_of_mass(bitmasks):
        ...
        # bitmasks.dtype = 'int'
        # m00 = bitmasks.sum(dim=-1).sum(dim=-1)
        m00 = np.sum(bitmasks) + 0.0001
        m10 = np.sum((bitmasks * xs))
        m01 = np.sum((bitmasks * ys[:, None]))
        center_x = m10 / m00
        center_y = m01 / m00
    ...
    scores = scores[0:2, ...]
    mask_colors = [
        np.array([0, 180, 0]).reshape(1, 3), np.array([180, 0, 0]).reshape(1, 3)
    ]

    polygons = []
    color = []
    for i, (segm, label, score) in enumerate(zip(segms, labels, scores)):
        if label == 2:
            continue
        label_text = class_names[
            label] if class_names is not None else f'class {label}'
        x, y = center_of_mass(segms[i])
        ax.text(
            x,
            y,
            f'{label_text}',
        ...
        # get the first point in the mask
