...
class ISDAHead2(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is DISTRFPNHead):  # Focal Loss 中没有设置
            assert isinstance(class_weight, float), 'Expected ' \
                                                    'class_weight to have type float. Found ' \
                                                    f'{type(class_weight)}.'
            # NOTE following the official DETR rep0, bg_cls_weight means
            # relative classification weight of the no-object class.
            bg_cls_weight = loss_cls.get('bg_cls_weight', class_weight)  # 如果没有找到就设置为class_weight
            assert isinstance(bg_cls_weight, float), 'Expected ' \
                                                     'bg_cls_weight to have type float. Found ' \
                                                     f'{type(bg_cls_weight)}.'
            class_weight = torch.ones(num_classes + 1) * class_weight  # 创建一个关于类别权重的loss
            # set background class as the last indice
            class_weight[num_classes] = bg_cls_weight
            loss_cls.update({'class_weight': class_weight})
            if 'bg_cls_weight' in loss_cls:
                loss_cls.pop('bg_cls_weight')  # 因为bg_cls_weight这个是在实例化loss类时没有用的
        ...
        if self.loss_cls.use_sigmoid:
            self.cls_out_channels = num_classes  # 使用sigmoid的话就只有80类，不然的话有81个类
        else:
    ...
    def _init_layers(self):
        fc_cls = Linear(self.embed_dims, self.cls_out_channels)  # 输出的类别判断

        kernel_branch = []
        for _ in range(self.num_kernel_fcs):
            kernel_branch.append(Linear(self.embed_dims, self.embed_dims))
            kernel_branch.append(nn.ReLU())
        kernel_branch.append(Linear(self.embed_dims, 254))  # 254 + 2 的目的是为了得到kernel branch
        ...
        self.kernel_branches = nn.ModuleList(
            [kernel_branch for _ in range(num_pred)])  # 新加的kernel分支
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for lvl in range(hs.shape[0]):  # 这里面的level表示的是不同的head
            if lvl == 0:
                reference = init_reference  # 第一个head部分是0
            else:
                reference = inter_references[lvl - 1]  # 最后一个会不会用上啊
            reference_temp = reference
            # reference = torch.zeros_like(reference)
            # reference = reference + reference.new_tensor([0.45 - 0.3761, 0.68 - 0.6027])
            # reference = reference + reference.new_tensor([0.1 - 0.3761, 0.4 - 0.6027])
            reference = inverse_sigmoid(reference)  # 反sigmoid
            ...
            if reference.shape[-1] == 4:  # 要么是偏移量，要么是
                tmp += reference
            else:
    ...
    def loss(self,
        ...
        losses_cls, losses_mask = multi_apply(
            ...
            all_gt_labels_list, all_gt_masks_list, img_metas_list, all_gt_bboxes_ignore_list)  # 获取的是每一个
    ...
    def loss_single(self,
        ...
        """
        num_imgs = cls_scores.size(0)  # 图片的数量
        ...
        if self.sync_cls_avg_factor:  # 这个是干什么用的？
            cls_avg_factor = reduce_mean(
                cls_scores.new_tensor([cls_avg_factor]))  # 加上试试看
    ...
    def _get_target_single(self,
        ...
        # 上面这些是为了得到预测的mask以及ground truth mask
        # rescale gt_masks
        # TODO:这里的解决还不是很优雅,采用weight的形式可能会更好, 这里这个GT需要修改
        if len(gt_masks) == 0:
        ...
        # 上面这部分是为了缩放GT的mask达到一般的大小，这个部分会不会进入计算图，应该是不会的
        ...
        mask_preds = mask_preds  # 不需要改动，直接传入100个
        ...
        labels = mask_preds.new_full((num_masks,),
                                     self.num_classes,
                                     dtype=torch.long)
        labels[pos_inds] = gt_labels[sampling_result.pos_assigned_gt_inds]  # 这个会进行对应
    ...
    def get_masks(self,
        ...
        cls_scores = all_cls_scores_list[-1]  # 获取最后一个decoder作为mask
    ...
    def _get_masks_single(self,
        ...
        # 这里只取前面的作为mask
        # exclude background
        # scores, det_labels = F.softmax(cls_score, dim=-1)[..., :-1].max(-1)  # 最后一个为背景，不包括在内

        if self.loss_cls.use_sigmoid:
            cls_score = cls_score.sigmoid()  # 第一步，先进行sigmoid
